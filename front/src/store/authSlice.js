...
// userSlice actions
export const loadMember = createAsyncThunk('auth/loadmember', async (role, { rejectWithValue }) => {
	try {
		if (role === CUSTOMER) {
...
export const modifyMember = createAsyncThunk('auth/modifymember', async (payload, { rejectWithValue }) => {
	try {
		let response;
...
export const modifyPass = createAsyncThunk('auth/modifypass', async (payload, { rejectWithValue }) => {
	try {
		let response;
		const data = {
		...
		builder
			...
			.addCase(UserInfo.fulfilled, (state, action) => {
				...
				if (action.payload.profile_img_url !== null) {
					...
					state.logonUser.imageUrl =
						'https://firebasestorage.googleapis.com/v0/b/sail106.appspot.com/o/anon.jpg?alt=media&token=c8378e56-f874-4051-beac-fa925e121143';
				}
				// role: action.payload.data
			})
