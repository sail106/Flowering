...
const Finalresult3step = () => {
  return (
    <Consulting1stepresultpage>
      ...
      <H3>전문가 솔루션</H3>
      <P2>
        저와 비슷한 경험이 있으신 분이였어요. 저도 어릴 때 저의 피부 장벽이
        무너진 줄 모르고 레이저며 시술이며 너무 많이 받고 좋다는 고기능성 제품을
        사용하다보니 더욱 더 예민해지며 트러블까지 일어났던 기억이 나네요. 가장
        메인 고민은, 어떻게 하면 피부가 건강해질까 이셨고 피부 건강을 위해서는
        가장 먼저 피부 장벽을 강화하는 것이 중요하기 때문에 피부 장벽을 튼튼하게
        해줄 제품들로 구성해보았습니다.
      </P2>
      <Margin2 />
      <InputSet>
      ...
      <H3>전문가 솔루션</H3>
      <P2>
        혜미님을 딱 보자마자 저는 복숭아가 떠올랐어요. 혜미님은 생기발랄한
        이미지를 갖고 계시기 때문에 따뜻하고 화사한 컬러를 활용하여 생기 넘치는
        메이크업을 추천드립니다. 이목구비가 뚜렷하시기 때문에 색조에 너무 큰
        힘을 주기보단 얼굴의 윤곽을 살리는 음영 메이크업을 추천드립니다.
      </P2>
      <Margin3 />
      <InputSet>
        <P>
      ...
      <InputSet>
        ...
        <H>아이 메이크업 : </H>
          ...
          일자형 눈썹과, 브러시를 사용하여 자연스러운 아이라인을 그립니다.
        </P>
      </InputSet>
      <Margin2 />
      <InputSet>
      ...
      <H3>전문가 솔루션</H3>
      <P>
        혜미님은 생기발랄한 이미지와 계란형 얼굴형을 갖고 계셔서 어울리는
        헤어스타일은 다양한데요. 일반적으로 계란형 얼굴에 어울리는 헤어스타일은
        얼굴을 부각시키고 균형을 잡아주는 스타일이 좋습니다. 예를 들어, 계란형
        얼굴에는 앞머리를 가볍게 내린 웨이브 헤어나 사이드 파트를 중심으로
        볼륨있는 스타일, 레이어드 컷 등이 어울립니다. 또한, 풍성한 볼륨의
        스타일이나 잔머리를 활용한 스타일도 계란형 얼굴에 잘 어울립니다.
...
export default Finalresult3step;
