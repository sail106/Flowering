...
public class FindRequest {
    ...
    public UserResponse toResponse(User user) {
        ...
        if (!this.isProfile_img_url() && user.getProfileImgUrl() != null)
            response.setProfileImgUrl(user.getProfileImgUrl());
        if (!this.isGender() && user.getGender() != null) response.setGender(user.getGender());
        if (!this.isBirthdate_year() && user.getBirthdateYear() != null)
            response.setBirthdateYear(user.getBirthdateYear());
        if (!this.isBirthdate_month() && user.getBirthdateMonth() != null)
            response.setBirthdateMonth(user.getBirthdateMonth());
        if (user.getName() == null || user.getBirthdateYear() == null || user.getBirthdateMonth() == null || user.getNickname() == null || user.getGender() == null)
            response.setHasAdditionalInfo(true);
        return response;
    }
}
